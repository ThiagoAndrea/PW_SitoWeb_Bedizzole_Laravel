    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-6 container-aggiungiGiocatore">
                ...
                @if (isset($giocatore->id_giocatore))
                    <form method="put"
                        action="{{ route('giocatore.update', ['id_giocatore' => $giocatore->id_giocatore]) }}"
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="id_squadra" name="id_squadra" required>
                        ...
                        <option value="7"
                            {{ isset($giocatore->id_squadra) && $giocatore->id_squadra == 7 ? 'selected' : '' }}>
                            Giovanissimi U14</option>
                        <option value="6"
                            {{ isset($giocatore->id_squadra) && $giocatore->id_squadra == 6 ? 'selected' : '' }}>Esordienti
                            U13</option>
                        <option value="5"
                            {{ isset($giocatore->id_squadra) && $giocatore->id_squadra == 5 ? 'selected' : '' }}>Esordienti
                            U12</option>
                        <option value="4"
                            {{ isset($giocatore->id_squadra) && $giocatore->id_squadra == 4 ? 'selected' : '' }}>Pulcini U11
                        </option>
                        <option value="3"
                            {{ isset($giocatore->id_squadra) && $giocatore->id_squadra == 3 ? 'selected' : '' }}>Pulcini U10
                        </option>
                        <option value="2"
                            {{ isset($giocatore->id_squadra) && $giocatore->id_squadra == 2 ? 'selected' : '' }}>Piccoli
                            Amici U9</option>
                        <option value="1"
                            {{ isset($giocatore->id_squadra) && $giocatore->id_squadra == 1 ? 'selected' : '' }}>Scuola
                            Calcio</option>
                    </select>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" id="nome" name="nome"
                                value="{{ isset($giocatore->nome) ? $giocatore->nome : '' }}" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" id="cognome" name="cognome"
                                value="{{ isset($giocatore->cognome) ? $giocatore->cognome : '' }}" required>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <input type="date" class="form-control" id="data_di_nascita" name="data_di_nascita"
                                value="{{ isset($giocatore->data_di_nascita) ? $giocatore->data_di_nascita : '' }}"
                                required>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select" id="ruolo" name="ruolo" required>
                                ...
                                <option value="Portiere"
                                    {{ isset($giocatore->ruolo) && $giocatore->ruolo == 'Portiere' ? 'selected' : '' }}>
                                    Portiere</option>
                                <option value="Difensore"
                                    {{ isset($giocatore->ruolo) && $giocatore->ruolo == 'Difensore' ? 'selected' : '' }}>
                                    Difensore</option>
                                <option value="Centrocampista"
                                    {{ isset($giocatore->ruolo) && $giocatore->ruolo == 'Centrocampista' ? 'selected' : '' }}>
                                    Centrocampista</option>
                                <option value="Attaccante"
                                    {{ isset($giocatore->ruolo) && $giocatore->ruolo == 'Attaccante' ? 'selected' : '' }}>
                                    Attaccante</option>
                            </select>
                        </div>
                    </div>
                ...
                @if (isset($giocatore) && $giocatore->foto)
                    <div class="mb-3">
                        <p>Immagine attuale:</p>
                        <img src="{{ isset($giocatore->foto) ? asset('img/giocatori/' . $giocatore->foto) : '' }}"
                ...
                <div class="d-flex justify-content-between">
                    @if (isset($giocatore->id_giocatore) && $giocatore != null)
                        ...
                        <button type="submit" class="btn btn-success"
                            onclick="event.preventDefault(); checkGiocatore()">Salva</button>
                    @else
                        <button type="button" class="btn btn-secondary" onclick="window.history.back()">Annulla</button>
                        <button type="submit" class="btn btn-success"
                            onclick="event.preventDefault(); checkGiocatore()">Aggiungi</button>
                    @endif
                </div>
                </form>
            </div>
